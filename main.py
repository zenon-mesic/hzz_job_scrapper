...
def insert_stats_and_date_into_html(
...
def create_html(
  ...
  if html == '':
    logging.warning(
      'Failed to insert DB rows into HTML. Row element not found'
    )
    return
...
def main() -> None:
  ...
  db_jobs, db_stats = update_database('jobs.db', xml_jobs)
  create_html(
    'templates/template.html', 'output/index.html', db_jobs, db_stats
  )
